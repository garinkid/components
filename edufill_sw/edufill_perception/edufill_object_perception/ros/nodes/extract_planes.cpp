...
public:
  ...
  {
    ros::NodeHandle nh;
    extract_planes_= nh.advertiseService("extract_planes", &ExtractPlanes::extract_planes, this);
    ROS_INFO("Started [extract_planes] service.");
...
private:
  ...
  ros::ServiceServer extract_planes_;
  edufill_srvs::ExtractPlanes::Response _last_planar_polygons;
  ...
  bool extract_planes(edufill_srvs::ExtractPlanes::Request  &req,edufill_srvs::ExtractPlanes::Response &res)
  {
    ...
    std::vector<hbrs_msgs::PlanarPolygon> PlanarPolygons;

    for (unsigned int j = 0; j < planar_polygons.size(); j++) {

	hbrs_msgs::PlanarPolygon planar_polygon;
  
        convertPlanarPolygon(planar_polygons[0], planar_polygon);

        PlanarPolygons.push_back(planar_polygon);	

    }
    _last_planar_polygons.stamp = ros::Time::now();
    _last_planar_polygons.planarpolygons = PlanarPolygons;

    res = _last_planar_polygons;
    return true;
  }
